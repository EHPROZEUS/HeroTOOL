...
const OilInfoForm = ({ oilInfo, updateOilInfo }) => {
  return (
    <div className="p-4 rounded-lg border" style={{ backgroundColor: colors.rowSelectedBg, borderColor: colors.brand }}>
      <h3 className="text-md font-semibold mb-3" style={{ color: colors.text }}>Informations huile moteur</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium uppercase tracking-wide mb-1" style={{ color: colors.textMuted }}>Viscosité</label>
          <select 
        ...
        <div>
          <label className="block text-xs font-medium uppercase tracking-wide mb-1" style={{ color: colors.textMuted }}>Quantité nécessaire (litres)</label>
          <input 
            type="text" 
      ...
      {oilInfo.viscosity && oilInfo.quantity && (
        <div className="mt-3 p-3 rounded-md" style={{ backgroundColor: '#F0F9FF', border: `1px solid ${colors.focus}` }}>
          <p className="text-sm font-semibold" style={{ color: colors.focus }}>
